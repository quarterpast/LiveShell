(function(){
  var fs, path, cp, q, Sync, repl, cwd, async, promiseStream, exec, registerBin, readDir, listBin, cd, __split = ''.split;
  fs = require('fs');
  path = require('path');
  ...
  Sync(function(){
    cd("lib");
    return echo('hello').stdout.pipe(process.stdout);
  });
