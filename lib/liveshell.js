(function(){
  var fs, path, cp, q, Sync, rl, LiveScript, vm, cwd, async, registerBin, readDir, listBin, run, exec, PS1, __split = ''.split;
  fs = require('fs');
  path = require('path');
  ...
  Sync(function(){
    ...
    return (function(){
      return this.question(PS1('matt', 'Vera', path.basename(cwd)), exec(ctx));
    }.call(rl.createInterface(process.stdin, process.stdout)));
  });
